...
function affiliateCard(f){
  if(isRecommended(f)){
    return `<div class="article-card affiliate-card"><h2>Recommended partner offer</h2><p>${recommendationBadge(f)} ${couponText(f)}</p><p>${f.name} has a partner offer listed here. Confirm the final price, discount terms, and current rules before buying.</p>${affiliateActions(f,false)}</div>`;
  }
  ...
  document.querySelector('#app').innerHTML = `
    <header class="nav">
      <div class="wrap nav-inner">
        ...
        <div class="nav-cta"><a class="btn" href="#checklist">Get checklist</a><a class="btn primary" href="#compare">Compare firms</a><button class="btn mobile-menu" id="menuBtn">Menu</button></div>
      </div>
    </header>
    <main>${content}</main>
...
function renderHome(){
  ...
  <section class="section"><div class="wrap"><div class="section-head"><div><h2>A futures prop firm shortlist built for NQ traders.</h2><p class="subhead">Start with the firms traders actually compare, then use NQ-focused calculators to understand drawdown, payout, and risk rules.</p></div><a class="btn" href="#firms">View firm pages</a></div><div class="grid3">
    ...
    <div class="card"><div class="icon">↗</div><h3>Trader-first comparison</h3><p>Built for traders who want clear rules, discount-code links, and practical account comparisons without signal-service hype.</p></div>
  </div></div></section>
...
function renderDisclaimers(){return `<article class="article wrap"><h1>Disclosures</h1><div class="article-card"><h2>Affiliate disclosure</h2><p>This site may earn commissions when visitors click partner links or use discount codes. Recommendations are based on rule fit, account structure, and trader risk. Partner compensation may exist, but it should not replace your own rule review.</p></div><div class="article-card"><h2>Educational only</h2><p>Funded Futures Tools is not financial advice, investment advice, or a promise that users will pass an evaluation or make money. Futures trading carries substantial risk.</p></div><div class="article-card"><h2>Rule-change policy</h2><p>Prop firm rules, prices, discounts, payout policies, and restrictions change frequently. Use the last-reviewed date and official-rule links as a starting point, then verify the latest terms directly with the firm.</p></div><div class="article-card"><h2>Before you use this site</h2><ul><li>Verify each prop firm rule directly from official sources before buying.</li><li>Recheck every partner offer, coupon code, and official rule page before purchasing an account.</li><li>Read the affiliate disclosure, educational disclaimer, and rule-change policy before relying on any comparison.</li><li>The checklist is provided as an educational planning aid, not a promise of account approval, payouts, or trading profits.</li></ul></div></article>`}
