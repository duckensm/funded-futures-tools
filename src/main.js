...
const firms = [
  ...
  {id:'alphafutures', name:'Alpha Futures', category:'One-step futures evaluations', best:'Alpha Futures account types and rules summarized from official sources', price:'Zero 50K listed at $119/mo; Standard 50K listed at $139/mo; try code Duckens026406 and confirm final checkout price', target:'One-step evaluations up to $750K simulated funds; 50K examples show $3,000 targets', drawdown:'Maximum Loss Limit / EOD drawdown rules vary by account type and size', daily:'Daily Loss Guard varies by account type and size; some account types list none', payout:'Performance fee requests after 5 winning days; winning day at least $200; manual review', risk:'Medium', officialUrl:'https://www.alpha-futures.com/', lastVerified:'2026-06-04', verification:'official', affiliateUrl:'https://app.alpha-futures.com/signup/Duckens026406/', couponCode:'Duckens026406', fit:'Best for traders comparing one-step futures evaluations, EOD-style drawdown language, account-type flexibility, and payout rules before checkout.'},
  {id:'daytraders', name:'DayTraders', category:'Futures-only evaluation, funded, and live paths', best:'DayTraders account types and drawdown models summarized from official sources', price:'Trailing $249-$879; EOD $309-$1,599; Static $150-$400; S2F $370-$825; S2L $329-$599; try code TNTIQNUL and confirm final checkout price', target:'Standard evaluations list 2 qualifying days; S2F/S2L have separate payout and live-account requirements', drawdown:'Trailing, EOD, Static, S2F, and S2L models; drawdown behavior depends on product type', daily:'Daily Loss Limits vary by program; EOD uses DLL as intraday cushion', payout:'Pro/S2F pages advertise 100% split; S2L live path uses 80/20 split; verify current payout requirements', risk:'Medium', officialUrl:'https://daytraders.com/', lastVerified:'2026-06-04', verification:'official', affiliateUrl:'https://daytraders.com/go/dutrading?c=TNTIQNUL', couponCode:'TNTIQNUL', fit:'Best for futures traders comparing one-time pricing, multiple drawdown models, S2F access, and S2L live-account progression.'},
  {id:'oneuptrader', name:'OneUp Trader', category:'One-step futures evaluation', best:'OneUp Trader pricing and rules summarized from official sources', price:'25K $65/mo; 50K $75/mo; 100K $150/mo; 150K $175/mo; 250K $325/mo; try code dutrading and confirm final checkout price', target:'$1.5k / $3k / $6k / $9k / $15k; 10 trading days or 5 days for Express', drawdown:'Trailing drawdown; stops trailing once it reaches the initial account balance', daily:'No daily loss limit listed on current official FAQ', payout:'First $10K profits 100%, then 90%; funded withdrawals from day one once threshold is reached', risk:'Medium', officialUrl:'https://www.oneuptrader.com/', lastVerified:'2026-06-05', verification:'official', affiliateUrl:'https://www.oneupapp.io/?via=dutrading', couponCode:'dutrading', fit:'Best for traders comparing a long-running one-step futures evaluation with no daily loss limit, trailing drawdown, and funded-account dynamic scaling rules.'},
  {id:'legendstrading', name:'The Legends Trading', category:'Apprentice, Elite, and Straight to Master', best:'The Legends Trading plans and payout rules summarized from official sources', price:'Current code offer: 50% OFF Apprentice Plans; 30% OFF Elite Plans; use code DUTRADING and confirm final checkout price', target:'Apprentice $1.5k/$3k/$6k/$9k; Elite $1.5k/$2.7k/$6k/$9k; Straight to Master varies by size', drawdown:'EOD trailing on Apprentice and Elite; Straight to Master lists trailing max loss', daily:'Elite evaluation lists no daily loss limit; funded and other plan rules vary', payout:'90/10 split; official payout policy lists up to twice monthly with trading-day and balance requirements', risk:'Medium', officialUrl:'https://thelegendstrading.com/', lastVerified:'2026-06-06', verification:'official', affiliateUrl:'https://thelegendstrading.com/?ref=dutrading', couponCode:'DUTRADING', fit:'Best for futures traders comparing a monthly Apprentice evaluation, one-time Elite evaluation, or a Straight-to-Master route with different fees and rule profiles.'},
...
function renderHome(){
  ...
  <section class="hero"><div class="wrap hero-grid">
    <div>
      ...
      <div class="mini-proof"><span><b>12</b> starter firms</span><span><b>3</b> calculators</span><span><b>NQ-first</b> examples</span><span><b>Current offers</b> clearly labeled</span></div>
    </div>
...
function renderFirms(){
  ...
  if(hash){
    ...
    if(f.id==='alphafutures') return renderAlphaFuturesArticle(f);
    if(f.id==='daytraders') return renderDayTradersArticle(f);
    if(f.id==='oneuptrader') return renderOneUpTraderArticle(f);
    if(f.id==='legendstrading') return renderLegendsTradingArticle(f);
...
function renderPhidiasArticle(f){
  ...
  <div class="article-card verify-card"><h2>Source review</h2><p><span class="pill ${verificationClass(f)}">${verificationLabel(f)}</span></p><p>Official sources: <a href="https://phidiaspropfirm.com/phidias-2-0" target="_blank" rel="noreferrer">Phidias 2.0 page</a>, <a href="https://phidiaspropfirm.com/accounts" target="_blank" rel="noreferrer">accounts</a>, <a href="https://phidiaspropfirm.com/rules" target="_blank" rel="noreferrer">rules</a>, and <a href="https://phidiaspropfirm.com/affiliate-program" target="_blank" rel="noreferrer">affiliate program</a>.</p><p class="disclaimer">Phidias rules and offers can change. Try code DUTRADING if you use the link, then confirm the final checkout price and current account rules before buying.</p></div>
  ${affiliateCard(f)}
...
function renderAlphaFuturesArticle(f){
  return `<article class="article wrap"><a class="btn" href="#firms">All firms</a><h1>Alpha Futures: one-step futures evaluations and account rules</h1><p class="lead">Alpha Futures is a futures evaluation firm with Zero, Premium, Advanced, and Standard challenge paths. Use this guide to compare account type, MLL, payout cap, news rules, and checkout price before buying.</p>
  <div class="article-card verify-card"><h2>Source review</h2><p><span class="pill ${verificationClass(f)}">${verificationLabel(f)}</span></p><p>Official sources: <a href="${f.officialUrl}" target="_blank" rel="noreferrer">Alpha Futures official site</a>, <a href="https://help.alpha-futures.com/en/articles/11927533-futures-challenge-rules" target="_blank" rel="noreferrer">challenge rules</a>, and <a href="https://help.alpha-futures.com/en/articles/11928417-when-can-i-request-a-performance-fee" target="_blank" rel="noreferrer">performance fee rules</a>.</p><p class="disclaimer">Confirm current account type, MLL, payout cap, news rules, and final checkout price on Alpha Futures before buying.</p></div>
  ${affiliateCard(f)}
  <div class="article-card"><h2>Alpha Futures account types</h2><p>The official rules page lists Zero, Premium, Advanced, and Standard account types. Zero and Premium show no consistency requirement in the published table. News trading differs by account type, so check the account you choose before trading major releases.</p><div class="table-wrap"><table><thead><tr><th>Account type</th><th>Rule angle</th><th>News trading</th><th>Notes to verify</th></tr></thead><tbody><tr><td>Zero</td><td>Most flexible published rule profile</td><td>Allowed in challenge and master account</td><td>No consistency listed; verify soft breach and position monitoring details</td></tr><tr><td>Premium</td><td>Flexible but stricter news rule</td><td>Not allowed</td><td>No consistency listed; verify payout cap and MLL for chosen size</td></tr><tr><td>Advanced</td><td>Lower displayed starting price than Premium/Zero</td><td>Verify official table</td><td>Confirm strategy, consistency, and payout-cap rules</td></tr><tr><td>Standard</td><td>Common comparison account type</td><td>Verify official table</td><td>Confirm rule restrictions before using NQ size</td></tr></tbody></table></div></div>
  <div class="article-card"><h2>Pricing and checkout notes</h2><ul><li>The official site shows multiple product tabs and account sizes. Current examples include Zero 50K at $119/month and Standard 50K at $139/month.</li><li>The homepage promotes one-step futures evaluations, up to $750K simulated funds, up to 6 accounts, EOD-style drawdown messaging, and no activation fee messaging on Zero accounts.</li><li>Use code ${f.couponCode} only after confirming the final checkout price, terms, and active promotion.</li></ul></div>
  <div class="article-card"><h2>Performance fee rules</h2><ul><li>Alpha Futures says performance fee requests can be made after at least 5 winning days.</li><li>A winning day must close in profit by at least $200.</li><li>The help center says there is no maximum request amount, but requests are manually reviewed before approval.</li><li>Performance fee processing time can depend on team workload, so verify the current payout page before relying on timing.</li></ul></div>
  <div class="article-card"><h2>NQ trader takeaways</h2><p>Alpha Futures is worth comparing if you want a one-step futures evaluation with EOD drawdown language, multiple account types, and no activation fee messaging on Zero accounts. For NQ traders, the account type matters more than the sticker price: verify MLL, payout cap, soft-breach rules, news restrictions, and whether your normal stop size fits the cushion.</p><a class="btn primary" href="#calculators">Run NQ risk calculator</a></div></article>`;
}


function renderDayTradersArticle(f){
  return `<article class="article wrap"><a class="btn" href="#firms">All firms</a><h1>DayTraders: futures-only prop firm paths and drawdown rules</h1><p class="lead">DayTraders offers several futures-only routes: standard evaluations, EOD accounts, Static accounts, Straight to Funded, and Straight to Live. Use this guide to compare the drawdown model before choosing a checkout link.</p>
  <div class="article-card verify-card"><h2>Source review</h2><p><span class="pill ${verificationClass(f)}">${verificationLabel(f)}</span></p><p>Official sources: <a href="${f.officialUrl}" target="_blank" rel="noreferrer">DayTraders official site</a>, <a href="https://daytraders.com/futures-prop-firms" target="_blank" rel="noreferrer">futures prop firm guide</a>, <a href="https://daytraders.com/help/articles/9855052-drawdown-threshold-guide-trailing-static-eod" target="_blank" rel="noreferrer">drawdown guide</a>, <a href="https://daytraders.com/help/articles/14038700-live-accounts-payouts" target="_blank" rel="noreferrer">live payouts</a>, and <a href="https://intercom.help/daytraders/en/articles/13642082-straight-to-funded-s2f-futures" target="_blank" rel="noreferrer">S2F rules</a>.</p><p class="disclaimer">Confirm current account type, drawdown model, payout terms, platform support, and final checkout price on DayTraders before buying.</p></div>
  ${affiliateCard(f)}
  <div class="article-card"><h2>DayTraders account paths</h2><p>The official futures prop firm guide describes Trailing, EOD, Static, S2F, and S2L paths. That variety is useful, but it also means traders need to compare the rule model, not just the displayed price.</p><div class="table-wrap"><table><thead><tr><th>Path</th><th>What it means</th><th>Price range shown</th><th>NQ trader note</th></tr></thead><tbody><tr><td>Trailing</td><td>Real-time trailing drawdown based on live balance, including unrealized PnL.</td><td>$249 to $879</td><td>Most sensitive to intraday runners and pullbacks.</td></tr><tr><td>EOD</td><td>End-of-day trailing drawdown that updates after the trading day closes.</td><td>$309 to $1,599</td><td>Often easier to manage than intraday trailing, but DLL still matters.</td></tr><tr><td>Static</td><td>Fixed floor that does not move with profit.</td><td>$150 to $400</td><td>Cleaner rule model, but check target, size, and promotion terms.</td></tr><tr><td>S2F</td><td>Straight to Funded simulated account without completing an evaluation first.</td><td>$370 to $825</td><td>Faster access, but payout eligibility and DLL rules still control risk.</td></tr><tr><td>S2L</td><td>Evaluation path that can lead to live funded accounts with real execution.</td><td>$329 to $599</td><td>Live path uses different payout split and platform support.</td></tr></tbody></table></div></div>
  <div class="article-card"><h2>Drawdown rules in plain English</h2><ul><li>Trailing accounts use intraday trailing drawdown that can move with unrealized profits.</li><li>Static accounts use a fixed drawdown floor that does not move with intraday or closed profit.</li><li>EOD accounts update the threshold once per day based on end-of-day balance.</li><li>Once an account is created, DayTraders says the drawdown type cannot be changed.</li><li>All accounts must stay above the threshold to remain active.</li></ul></div>
  <div class="article-card"><h2>Payout and live-account notes</h2><ul><li>The official guide says Pro and S2F accounts keep 100% of profits, while live funded accounts use an 80/20 split.</li><li>Live payout requests require the trailing drawdown to lock at the initial balance.</li><li>Live accounts require a $1,000 buffer above initial balance before unrestricted withdrawal.</li><li>Live payout processing is listed daily between 5:00 and 6:00 PM ET, subject to account conditions.</li><li>S2F accounts are simulated, not live brokerage accounts, and use account-specific payout eligibility rules.</li></ul></div>
  <div class="article-card"><h2>NQ trader takeaways</h2><p>DayTraders is worth comparing because it gives traders multiple futures-only paths instead of one rigid model. For NQ and MNQ traders, the most important choice is drawdown type: intraday trailing can punish unrealized spikes, EOD is easier to plan around, and Static is the simplest floor. Use the risk calculator before buying so the stop size fits the selected account cushion.</p><a class="btn primary" href="#calculators">Run NQ risk calculator</a></div></article>`;
}


function renderOneUpTraderArticle(f){
  return `<article class="article wrap"><a class="btn" href="#firms">All firms</a><h1>OneUp Trader: one-step futures evaluation rules for NQ traders</h1><p class="lead">OneUp Trader is a long-running futures evaluation program with monthly account sizes from 25K to 250K, no daily loss limit listed in the current FAQ, and a trailing drawdown model that stops at the starting balance.</p>
  <div class="article-card verify-card"><h2>Source review</h2><p><span class="pill ${verificationClass(f)}">${verificationLabel(f)}</span></p><p>Official sources: <a href="${f.officialUrl}" target="_blank" rel="noreferrer">OneUp Trader official site</a>, <a href="https://help.oneuptrader.com/article/387-what-are-the-rules-of-the-evaluation" target="_blank" rel="noreferrer">evaluation rules</a>, <a href="https://help.oneuptrader.com/article/372-funded-trader-rules" target="_blank" rel="noreferrer">funded trader rules</a>, <a href="https://help.oneuptrader.com/article/468-most-popular-questions" target="_blank" rel="noreferrer">FAQ</a>, and <a href="https://help.oneuptrader.com/article/386-funded-trader-dynamic-scaling-targets" target="_blank" rel="noreferrer">dynamic scaling targets</a>.</p><p class="disclaimer">Confirm current account size, trailing drawdown, consistency, funded rules, reset fees, and final checkout price on OneUp Trader before buying.</p></div>
  ${affiliateCard(f)}
  <div class="article-card"><h2>OneUp Trader account sizes</h2><p>The official site lists five monthly evaluation sizes. Pricing and promos can change, so treat these as current reference points and confirm the final checkout price after trying code ${f.couponCode}.</p><div class="table-wrap"><table><thead><tr><th>Account</th><th>Price shown</th><th>Profit target</th><th>Trailing drawdown</th><th>Max contracts</th></tr></thead><tbody><tr><td>25K</td><td>$65/mo</td><td>$1,500</td><td>$1,500</td><td>3</td></tr><tr><td>50K</td><td>$75/mo</td><td>$3,000</td><td>$2,500</td><td>6</td></tr><tr><td>100K</td><td>$150/mo</td><td>$6,000</td><td>$3,500</td><td>12</td></tr><tr><td>150K</td><td>$175/mo</td><td>$9,000</td><td>$5,000</td><td>15</td></tr><tr><td>250K</td><td>$325/mo</td><td>$15,000</td><td>$5,500</td><td>25</td></tr></tbody></table></div></div>
  <div class="article-card"><h2>Evaluation rules</h2><ul><li>One-step evaluation with at least 10 trading days or 5 days for Express accounts.</li><li>The evaluation requires reaching the selected account profit target without hitting the max trailing drawdown.</li><li>Current FAQ says there is no daily loss limit on evaluation or funded accounts.</li><li>The consistency rule requires three other best trading days to total at least 80% of the largest winning day.</li><li>Positions must be closed by 3:15 PM CT during the evaluation trading session.</li><li>Permitted products include futures markets such as NQ, MNQ, ES, CL, GC, and many others.</li></ul></div>
  <div class="article-card"><h2>Funded-account rules</h2><ul><li>Funded traders must follow allowed instruments, permitted times, trailing drawdown, dynamic scaling targets, and major-news restrictions.</li><li>Major economic releases require funded traders to be flat before, during, and after the release window.</li><li>Funded traders must maintain weekly trade volume relative to the evaluation and keep positive net PnL during the initial 90-day probation windows.</li><li>Funded traders may have up to three funded accounts; Express is limited to one funded account at a time.</li><li>Dynamic scaling means funded contract size depends on cash on hand at the beginning of the session.</li></ul></div>
  <div class="article-card"><h2>Payout and reset notes</h2><ul><li>The official site says funded traders keep 100% of the first $10,000 withdrawn and 90% after that.</li><li>The FAQ says withdrawals are available from day one once the withdrawal threshold is reached, with a $1,000 minimum and no maximum limit.</li><li>Withdrawal methods listed include bank wire and cryptocurrency.</li><li>Reset prices listed in the FAQ: $50 for 25K and 50K accounts, $100 for all other account sizes.</li><li>R|Trader Pro, evaluation market data, and NinjaTrader license messaging are listed as free, but platform costs can become the trader's responsibility once funded.</li></ul></div>
  <div class="article-card"><h2>NQ trader takeaways</h2><p>OneUp Trader can fit NQ/MNQ traders who want a straightforward one-step evaluation and no daily loss limit, but the trailing drawdown still matters. The 50K account gives 6 max contracts, but that does not mean 6 NQ is sensible early in the account. Use MNQ or smaller NQ size until the drawdown has room and the consistency rule is no longer the main bottleneck.</p><a class="btn primary" href="#calculators">Run NQ risk calculator</a></div></article>`;
}


function renderLegendsTradingArticle(f){
  return `<article class="article wrap"><a class="btn" href="#firms">All firms</a><h1>The Legends Trading: Apprentice, Elite, and Straight-to-Master rules</h1><p class="lead">The Legends Trading offers three futures funding paths: monthly Apprentice evaluations, one-time Elite evaluations, and Straight to Master accounts. The right choice depends on whether you prioritize monthly cost, no activation fee, or skipping the evaluation stage.</p>
  <div class="article-card verify-card"><h2>Source review</h2><p><span class="pill ${verificationClass(f)}">${verificationLabel(f)}</span></p><p>Official sources: <a href="${f.officialUrl}" target="_blank" rel="noreferrer">The Legends Trading official site</a>, <a href="https://thelegendstrading.com/plans" target="_blank" rel="noreferrer">plans</a>, <a href="https://knowledge.thelegendstrading.com/" target="_blank" rel="noreferrer">knowledge base</a>, and <a href="https://knowledge.thelegendstrading.com/payout-process" target="_blank" rel="noreferrer">payout policy</a>.</p><p class="disclaimer">Confirm the DUTRADING discount, account rules, activation fee, payout terms, and final checkout price on The Legends Trading before buying. The offer supplied for this link is 50% OFF Apprentice Plans and 30% OFF Elite Plans; the public site may display a separate sitewide offer.</p></div>
  ${affiliateCard(f)}
  <div class="article-card"><h2>Current DUTRADING promotion</h2><ul><li>50% OFF Apprentice Plans.</li><li>30% OFF Elite Plans.</li><li>Code to try: ${f.couponCode}.</li><li>Open the link first, apply the code at checkout, and confirm the actual discount before paying.</li></ul></div>
  <div class="article-card"><h2>Apprentice, Elite, and Straight to Master</h2><div class="table-wrap"><table><thead><tr><th>Path</th><th>Payment model</th><th>Drawdown</th><th>Consistency</th><th>Minimum days</th><th>Activation</th></tr></thead><tbody><tr><td>Apprentice</td><td>Monthly subscription</td><td>EOD trailing max loss</td><td>30%</td><td>4 days shown</td><td>$99 to $199 by size</td></tr><tr><td>Elite</td><td>One-time evaluation</td><td>EOD trailing max loss</td><td>40%</td><td>Verify current plan page</td><td>None shown</td></tr><tr><td>Straight to Master</td><td>One-time purchase</td><td>Trailing max loss</td><td>30%</td><td>10 days shown</td><td>Verify checkout and funded terms</td></tr></tbody></table></div></div>
  <div class="article-card"><h2>Apprentice account reference</h2><div class="table-wrap"><table><thead><tr><th>Size</th><th>Regular monthly price shown</th><th>Profit target</th><th>EOD max loss</th><th>Max contracts / micros</th><th>Activation fee</th></tr></thead><tbody><tr><td>25K</td><td>$165</td><td>$1,500</td><td>$1,500</td><td>4 / 40</td><td>$99</td></tr><tr><td>50K</td><td>$185</td><td>$3,000</td><td>$2,000</td><td>10 / 50</td><td>$99</td></tr><tr><td>100K</td><td>$225</td><td>$6,000</td><td>$3,000</td><td>14 / 70</td><td>$149</td></tr><tr><td>150K</td><td>$320</td><td>$9,000</td><td>$4,000</td><td>17 / 85</td><td>$199</td></tr></tbody></table></div><p class="disclaimer">Apply DUTRADING and confirm the 50% Apprentice discount at checkout. Do not calculate expected billing from the table without checking the final order total.</p></div>
  <div class="article-card"><h2>Elite account reference</h2><div class="table-wrap"><table><thead><tr><th>Size</th><th>Regular one-time price shown</th><th>Profit target</th><th>EOD max loss</th><th>Daily loss limit</th><th>Consistency</th></tr></thead><tbody><tr><td>25K</td><td>$99</td><td>$1,500</td><td>$1,250</td><td>None</td><td>40%</td></tr><tr><td>50K</td><td>$149</td><td>$2,700</td><td>$2,200</td><td>None</td><td>40%</td></tr><tr><td>100K</td><td>$180</td><td>$6,000</td><td>$3,000</td><td>None</td><td>40%</td></tr><tr><td>150K</td><td>$230</td><td>$9,000</td><td>$4,500</td><td>None</td><td>40%</td></tr></tbody></table></div><p class="disclaimer">Apply DUTRADING and confirm the 30% Elite discount at checkout. Elite rules differ from Apprentice rules, especially consistency and activation-fee treatment.</p></div>
  <div class="article-card"><h2>Payout notes</h2><ul><li>The official payout policy lists a 90/10 trader/company split.</li><li>Payout requests may be made up to twice per month during listed request windows.</li><li>The first payout requires 10 individual active trading days.</li><li>Later payout requests require at least 10 active trading days between requests.</li><li>The first and second payouts list a $1,000 minimum, with account-size balance thresholds and payout caps.</li><li>The account must be above the trailing drawdown level and meet the current consistency rule before a request can be approved.</li></ul></div>
  <div class="article-card"><h2>NQ trader takeaways</h2><p>The Legends Trading gives NQ traders several ways to enter, but the plan labels hide meaningful rule differences. Apprentice has monthly billing, 30% consistency, and activation fees. Elite uses one-time pricing, no evaluation daily loss limit, 40% consistency, and no activation fee shown. Straight to Master skips the normal challenge path but has its own trailing-loss and minimum-day rules. Compare the full cost and size NQ against the actual drawdown, not the advertised maximum contract count.</p><a class="btn primary" href="#calculators">Run NQ risk calculator</a></div></article>`;
}
