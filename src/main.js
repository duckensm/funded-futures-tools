...
const firms = [
  ...
  {id:'topstep', name:'Topstep', best:'Credibility / brand trust', price:'$49+', target:'Account dependent', drawdown:'Trailing / daily loss', daily:'Yes', payout:'Mature program', score:82, risk:'Medium-Low', fit:'Best for beginners who want established rules and education.'},
...
function renderHome(){
  ...
  <section class="hero"><div class="wrap hero-grid">
    <div>
      ...
      <div class="mini-proof"><span><b>6</b> starter firms</span><span><b>3</b> calculators</span><span><b>NQ-first</b> examples</span><span><b>No signals</b>, just rules + risk</span></div>
    </div>
...
function calculatorSection(){
  return `<section class="section" id="calculators"><div class="wrap"><div class="section-head"><div><h2>Free calculators that create trust.</h2><p class="subhead">These tools are the traffic hook: they make the site useful before affiliate links are even approved.</p></div></div><div class="calc-grid"><div class="tabs"><button class="tab active" data-calc="drawdown"><b>Trailing drawdown</b><br>Find liquidation threshold and cushion.</button><button class="tab" data-calc="nq"><b>NQ / MNQ risk</b><br>Translate stops and contracts into dollars.</button><button class="tab" data-calc="planner"><b>Challenge pass planner</b><br>Estimate daily target and risk pace.</button></div><div class="calculator" id="calcPanel"></div></div></div></section>`;
}
function renderCalculators(){ return calculatorSection(); }

function drawdownCalc(){return `<h3>Trailing drawdown simulator</h3><p class="disclaimer">Enter the account state. For exact rules, verify whether the firm uses intraday trailing, end-of-day trailing, or static drawdown.</p><div class="form-grid"><div><label>Starting balance</label><input id="startBal" type="number" value="50000"></div><div><label>Current balance</label><input id="currentBal" type="number" value="51400"></div><div><label>High-water mark</label><input id="highBal" type="number" value="52400"></div><div><label>Drawdown amount</label><input id="ddAmount" type="number" value="2500"></div><div><label>Drawdown type</label><select id="ddType"><option value="trailing">Trailing</option><option value="static">Static from start</option></select></div></div><div class="result-box" id="ddResults"></div><div class="note" id="ddNote"></div>`}
...
function updateCalc(which){
  if(which==='drawdown'){
    ...
    document.getElementById('ddResults').innerHTML=`<div class="metric"><span>Liquidation threshold</span><strong>${money(threshold)}</strong></div><div class="metric"><span>Remaining cushion</span><strong>${money(cushion)}</strong></div><div class="metric"><span>Drawdown used</span><strong>${used.toFixed(0)}%</strong></div>`;
    document.getElementById('ddNote').textContent = cushion <= 0 ? 'This account would be at or below the failure threshold.' : cushion < dd*.25 ? 'Danger zone: one normal NQ loss could put this account near failure.' : 'Cushion looks workable, but keep size aligned with the remaining drawdown.';
  }
  if(which==='nq'){
