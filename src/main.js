...
const firms = [
  ...
  {id:'tradeify', name:'Tradeify', category:'Best flexible payout-path option', best:'Official homepage/help rules checked in Chrome', price:'Growth promo from $59; Select promo from $65; Lightning promo from $207; verify live code', target:'$1.5k / $3k / $6k / $9k', drawdown:'EOD trailing on Growth, Select, and Lightning; funded drawdown locks at start + $100', daily:'Growth/Lightning DLL applies; Select eval none; Select Flex funded none; Select Daily funded $500/$1k/$1.25k/$1.75k', payout:'90% split; Growth 5 winning days + 35% funded consistency; Select Flex 5 winning days; Select Daily daily eligibility; Lightning 20% consistency', risk:'Medium', officialUrl:'https://tradeify.co/', lastVerified:'2026-05-31', verification:'official', affiliateUrl:'', couponCode:'', fit:'Best for NQ traders who want EOD drawdown, no activation fee, and a choice between Growth speed, Select payout flexibility, or Lightning instant simulated funding.'},
  {id:'bulenox', name:'Bulenox', category:'Recommended — budget account + EOD/trailing choice', best:'Official homepage/help rules checked', price:'$115/$145/$125 promo/$155 promo/$325/$535 monthly; reset $78; verify dutrading at checkout', target:'$1k / $1.5k / $3k / $6k / $9k / $15k', drawdown:'Option 1 intraday trailing/no scaling; Option 2 EOD with scaling + DLL', daily:'EOD option DLL: $400/$500/$1.1k/$2.2k/$3.3k/$4.5k', payout:'First $10k 100%; then 90%; Master payouts weekly after 10 trading days; 40% consistency', risk:'High', officialUrl:'https://bulenox.com/', lastVerified:'2026-05-31', verification:'official', affiliateUrl:'https://bulenox.com/member/aff/go/dutrading', couponCode:'dutrading', fit:'Best for deal seekers who want low displayed subscription prices and are willing to read the two drawdown options, activation fees, and Master payout rules carefully.'},
  {id:'earn2trade', name:'Earn2Trade', category:'Legacy structured evaluation + live-account path', best:'Official purchase/product pages checked', price:'TCP $150/$190/$350 mo; Gauntlet Mini $170/$315/$375/$550 mo; promo banner says 60% off', target:'TCP $1.75k/$3k/$6k; Gauntlet $3k/$6k/$9k/$11k', drawdown:'Evaluation/LiveSim EOD drawdown; Live account trailing drawdown; TCP can scale to $400K path', daily:'$550/$1.1k/$2.2k/$3.3k/$4.4k depending size', payout:'80% profit split; weekly withdrawals from $100+; $139 activation deducted from first successful withdrawal', risk:'Medium', officialUrl:'https://www.earn2trade.com/', lastVerified:'2026-05-31', verification:'official', affiliateUrl:'', couponCode:'', fit:'Best as a legacy structured route for traders who value fixed progression, real/live-account optionality, education resources, and a clear 10-day minimum evaluation.'},
  {id:'takeprofittrader', name:'TakeProfitTrader', category:'Brief reference only', best:'Brief reference; kept for rule comparison only', price:'See official site', target:'50K target verified at $3,000; other sizes verify on official site', drawdown:'Test EOD; PRO intraday; PRO+ EOD', daily:'Rules vary by stage; verify official help center', payout:'Brief reference only; verify payout rules on official site', risk:'Medium-High', officialUrl:'https://takeprofittrader.com/', lastVerified:'2026-05-31', verification:'official', affiliateUrl:'', couponCode:'', fit:'Brief reference only. TakeProfitTrader is kept at the bottom for comparison context and is not a current recommendation.'}
];
...
function affiliatePrimaryLabel(f){ return f.affiliateUrl ? (f.couponCode ? `Check discount ${f.couponCode}` : 'Check partner offer') : (f.id === 'takeprofittrader' ? 'Official site only' : 'Visit official site'); }
...
function affiliateCard(f){
  if(isRecommended(f)){
    return `<div class="article-card affiliate-card"><h2>Recommended partner offer</h2><p>${recommendationBadge(f)} ${couponText(f)}</p><p>${f.name} is a current partner offer on this site. Recheck live checkout prices, discount terms, and current rules before buying.</p>${affiliateActions(f,false)}</div>`;
  }
    ...
    <header class="nav">
      <div class="wrap nav-inner">
        ...
        <nav class="nav-links" aria-label="Primary">
          <a href="#compare">Compare Firms</a><a href="#checklist">Checklist</a><a href="#calculators">Calculators</a><a href="#firms">Firm Pages</a><a href="#disclaimers">Disclosures</a>
        </nav>
        <div class="nav-cta"><a class="btn" href="#checklist">Get checklist</a><a class="btn primary" href="#compare">Find best firm</a><button class="btn mobile-menu" id="menuBtn">Menu</button></div>
      </div>
    </header>
    <main>${content}</main>
...
function calculatorSection(){
  return `<section class="section" id="calculators"><div class="wrap"><div class="section-head"><div><h2>Free calculators that create trust.</h2><p class="subhead">Use these calculators to pressure-test drawdown, consistency, futures stop risk, and challenge pace before choosing an account.</p></div></div><div class="calc-grid"><div class="tabs"><button class="tab active" data-calc="drawdown"><b>Trailing drawdown + consistency</b><br>Calculate liquidation cushion and 30/40/50% consistency rules.</button><button class="tab" data-calc="nq"><b>Futures risk</b><br>NQ, MNQ, ES, GC, and CL stop-risk calculator.</button><button class="tab" data-calc="planner"><b>Challenge pass planner</b><br>Estimate daily target and risk pace.</button></div><div class="calculator" id="calcPanel"></div></div></div></section>`;
}
...
function renderFirms(){
  ...
  if(hash){
    ...
    return `<article class="article wrap"><a class="btn" href="#firms">← All firms</a><h1>${f.name} for futures traders</h1><p class="lead">${f.fit}</p><div class="article-card verify-card"><h2>Verification status</h2><p><span class="pill ${verificationClass(f)}">${verificationLabel(f)}</span></p><p>Official source to check: <a href="${f.officialUrl}" target="_blank" rel="noreferrer">${f.officialUrl}</a></p><p class="disclaimer">This firm is a brief research reference until its official rules are fully checked and this page has a real last-verified date.</p></div><div class="article-card"><h2>Rules in plain English</h2><p>This page is a verification template. Before treating this as a recommendation, verify current evaluation prices, profit targets, drawdown mechanics, payout rules, activation fees, reset fees, consistency rules, contract limits, and prohibited strategies on the official ${f.name} website.</p><ul><li>Category: ${f.category}</li><li>Typical risk level: ${f.risk}</li><li>Price: ${f.price}</li><li>Profit target: ${f.target}</li><li>Drawdown note: ${f.drawdown}</li><li>Daily loss: ${f.daily}</li><li>Payout note: ${f.payout}</li></ul></div><div class="article-card"><h2>How NQ traders usually fail</h2><p>The common failure pattern is oversizing NQ contracts while the trailing drawdown is still close to the current balance. Use MNQ until the cushion is large enough, then scale only after the account has room.</p><a class="btn primary" href="#calculators">Run calculator for this account</a></div></article>`;
  }
  return `<section class="section"><div class="wrap"><div class="section-head"><div><h2>Firm verification queue.</h2><p class="subhead">These pages are now organized by buyer-intent category and visibly marked until official rules are checked.</p></div></div><div class="firm-grid">${firms.map(f=>`<a class="firm" href="#firms/${f.id}"><div class="firm-top"><div><h3>${f.name}</h3><p>${f.fit}</p></div><span class="status-dot ${verificationClass(f)}"></span></div><div class="firm-meta"><span class="pill ${verificationClass(f)}">${verificationLabel(f)}</span>${recommendationBadge(f)}<span class="pill">${f.category}</span></div></a>`).join('')}</div></div></section>`;
}
...
function renderPhidiasArticle(f){
  ...
  <div class="article-card verify-card"><h2>Verification status</h2><p><span class="pill ${verificationClass(f)}">${verificationLabel(f)}</span></p><p>Official sources checked: <a href="https://phidiaspropfirm.com/phidias-2-0" target="_blank" rel="noreferrer">Phidias 2.0 launch</a>, <a href="https://phidiaspropfirm.com/accounts" target="_blank" rel="noreferrer">accounts</a>, <a href="https://phidiaspropfirm.com/rules" target="_blank" rel="noreferrer">rules</a>, and <a href="https://phidiaspropfirm.com/affiliate-program" target="_blank" rel="noreferrer">affiliate program</a>.</p><p class="disclaimer">Official Phidias pages were captured on 2026-06-01. Partner link and code DUTRADING were added on 2026-06-02; visible promo/coupon language and checkout pricing can change, so recheck checkout before sending paid traffic.</p></div>
  ${affiliateCard(f)}
...
function renderBulenoxArticle(f){
  ...
  <div class="article-card verify-card"><h2>Verification status</h2><p><span class="pill ${verificationClass(f)}">${verificationLabel(f)}</span></p><p>Official source checked: <a href="${f.officialUrl}" target="_blank" rel="noreferrer">${f.officialUrl}</a></p><p class="disclaimer">Official Bulenox homepage and Help Center pages were checked on 2026-05-31. Coupon prices shown on the homepage should be rechecked in live checkout before paid traffic.</p></div>
  ${affiliateCard(f)}
  <div class="article-card"><h2>Homepage account tiers captured</h2><div class="table-wrap"><table><thead><tr><th>Account</th><th>Max contracts</th><th>Profit target</th><th>Drawdown</th><th>Monthly price shown</th></tr></thead><tbody><tr><td>10K</td><td>5 micros</td><td>$1,000</td><td>$1,000</td><td>$115/month</td></tr><tr><td>25K</td><td>3 contracts</td><td>$1,500</td><td>$1,500</td><td>$145/month</td></tr><tr><td>50K</td><td>7 contracts</td><td>$3,000</td><td>$2,500</td><td>$125/month with $50OFF coupon; $175 listed before coupon</td></tr><tr><td>100K</td><td>12 contracts</td><td>$6,000</td><td>$3,000</td><td>$155/month with $60OFF coupon; $215 listed before coupon</td></tr><tr><td>150K</td><td>15 contracts</td><td>$9,000</td><td>$4,500</td><td>$325/month</td></tr><tr><td>250K</td><td>25 contracts</td><td>$15,000</td><td>$5,500</td><td>$535/month</td></tr></tbody></table></div><p class="disclaimer">Each tier offered “Opt 1: No Scaling Account” or “Opt 2: EOD Account” on the official homepage capture.</p></div>
  <div class="article-card"><h2>Qualification Account rules checked</h2><ul><li>No minimum trading days are required to get a Master Account; the trader must reach the profit target and have at least one completed trading day for review.</li><li>All open positions must be closed before 15:59 CT/CST.</li><li>NQ and MNQ are listed as permitted futures instruments.</li><li>One standard contract equals 10 micro contracts.</li><li>Using trade copiers, algorithms, strategies, or bots is not forbidden, but Bulenox says it is not responsible for third-party software issues.</li><li>Reset cost is $78 outside the free reset on billing date; reset does not change the subscription expiration date.</li></ul></div>
  <div class="article-card"><h2>Two drawdown options checked</h2><ul><li><b>Option 1 No Scaling:</b> trailing drawdown follows current balance, including realized and unrealized gains, records in real time, and includes commissions.</li><li><b>Option 2 EOD:</b> EOD drawdown updates when the account reaches a new high balance at the end of the trading day.</li><li>For Master Accounts, trailing/EOD drawdown stops moving when it reaches the initial starting balance + $100.</li><li>EOD Daily Loss Limits: 10K $400, 25K $500, 50K $1,100, 100K $2,200, 150K $3,300, 250K $4,500.</li><li>DLL is based on P&L including commissions and real-time/unrealized trades. Hitting it suspends the account for the rest of the trading day and is not counted as a rule violation.</li></ul></div>
  <div class="article-card"><h2>EOD scaling and Master fees checked</h2><ul><li>10K EOD has no scaling and max 5 micro contracts.</li><li>25K EOD scales from 2 contracts to 3 contracts after $1,501+ cash on hand.</li><li>50K EOD scales 2 → 4 → 7 contracts as cash on hand reaches $1,501 and $4,001+.</li><li>100K EOD scales 3 → 5 → 8 → 12 contracts as cash on hand reaches $2,001, $3,001, and $5,001+.</li><li>150K EOD scales 5 → 8 → 10 → 15 contracts as cash on hand reaches $4,001, $8,001, and $12,001+.</li><li>Master activation fees captured: 25K $143, 50K $148, 100K $248, 150K $498, 250K $898. The Qualification page also lists 10K at $98 for Option 2.</li></ul></div>
  <div class="article-card"><h2>Master and Funded payout rules checked</h2><ul><li>Master payouts: first $10,000 earned is paid 100% to the trader; after the first $10,000, trader receives 90% and Bulenox commission is 10%.</li><li>Master payout requests can be submitted any time during the calendar month, and payouts are processed weekly on Wednesdays.</li><li>Master payout processing requires at least 10 individual trading days.</li><li>Master minimum withdrawal is $1,000; first-three-payout maximums: 25K $1,000, 50K $1,500, 100K $1,750, 150K $2,000, 250K $2,500. After the third payout there is no maximum withdrawal limit.</li><li>Master payout consistency is 40%: the best single trading day must not exceed 40% of total profit balance at withdrawal request.</li><li>After three successful Master payouts, a trader may transition to a Funded Account at Risk Management's sole discretion. Funded Account reward request requires at least 5 trading days.</li></ul></div>
  <div class="article-card"><h2>Official Bulenox pages checked</h2><ul><li>https://bulenox.com/</li><li>https://bulenox.com/help/qualification-account/</li><li>https://bulenox.com/help/master-account/</li><li>https://bulenox.com/help/funded-account/</li><li>https://bulenox.com/help/subscription-and-payment/</li><li>https://bulenox.com/help/frequently-asked-questions/</li></ul></div>
...
function renderTakeProfitTraderArticle(f){
  ...
  <div class="article-card verify-card"><h2>Verification status</h2><p><span class="pill ${verificationClass(f)}">${verificationLabel(f)}</span></p><p>Official source checked: <a href="${f.officialUrl}" target="_blank" rel="noreferrer">${f.officialUrl}</a></p><p class="disclaimer">This page is intentionally minimal. Use the official site for current prices, rules, promos, and account availability.</p></div>
  ${affiliateCard(f)}
  <div class="article-card"><h2>Quick notes only</h2><ul><li>Known for day-one PRO payout messaging after buffer requirements are met.</li><li>Test drawdown was checked as EOD; PRO-stage drawdown can be intraday trailing; PRO+ uses EOD.</li><li>Consistency, buffer, reset, payout, and news rules should be verified directly before buying.</li></ul></div>
  <div class="article-card"><h2>Why it is at the bottom</h2><p>This site is currently prioritizing Lucid Trading and Bulenox as recommended partner options. TakeProfitTrader remains only as a comparison footnote for traders who already planned to research it.</p><a class="btn" href="#compare">Back to comparison</a></div></article>`;
}
...
function bindLeadForm(){
  ...
  const unlock=()=>{
    ...
    status.textContent='Checklist unlocked. Save the download and keep using the comparison/finder before choosing a firm.';
  };
  ...
  downloadBtn?.addEventListener('click',()=>{
    const blob=new Blob([checklistText(email.value.trim(), focus.value)],{type:'text/plain'});
    const url=URL.createObjectURL(blob);
    const a=document.createElement('a');
    a.href=url;
    a.download='nq-prop-firm-risk-checklist.txt';
    document.body.appendChild(a);
    a.click();
    a.remove();
    URL.revokeObjectURL(url);
    trackEvent('lead_magnet_download',{focus:focus.value,path:location.hash||'#checklist'});
  });
}
