...
function affiliateCard(f){
  if(isRecommended(f)){
    return `<div class="article-card affiliate-card"><h2>Recommended partner offer</h2><p>${recommendationBadge(f)} ${couponText(f)}</p><p>Lucid Trading and Bulenox are the current recommended partner links on this site. Recheck live checkout prices and discount terms before buying.</p>${affiliateActions(f,false)}</div>`;
  }
